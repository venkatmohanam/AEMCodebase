            ...
            data: { j_username: $("#userId").val(), j_password: $("#inputPassword").val(),j_validate: "true" },
                    ...
                    complete: function(xhr, textStatus) {
                       console.log(xhr.status);
                        //if status code is 403, the login has failed. this esponse will not go to success block, so capture here and show error message
                        if(xhr.status == 403){
                       	 $("#errordiv").html("Invalid User Name or Password");
                        	$("#errordiv").show();
                        }
                    } 
            });
...
var getRedirectPath = function () {
    return "/content/AEMMaster/en/aemworld.html";
}
